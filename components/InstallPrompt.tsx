    ...
    <>
          ...
          <div className="bg-white dark:bg-surface-dark p-4 rounded-[2rem] shadow-2xl border-2 border-primary/20 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-text-main-light dark:text-white">Acesso rápido e offline!</p>
              </div>
            </div>
            <div className="flex gap-2">
               <button onClick={() => setShowAndroidPrompt(false)} className="px-3 py-2 text-[10px] font-black text-gray-400 uppercase">Agora não</button>
      ...
      {showIosPrompt && (
        <div className="fixed bottom-24 left-6 right-6 z-[100] animate-in fade-in duration-500">
          <div className="bg-white/90 dark:bg-surface-dark/90 backdrop-blur-md p-6 rounded-[2.5rem] shadow-2xl border-2 border-primary/20 flex flex-col items-center text-center">
            <div className="size-14 rounded-[1.5rem] bg-primary mb-4 flex items-center justify-center shadow-lg">
              <span className="material-symbols-outlined text-white text-3xl">add_to_home_screen</span>
            </div>
            <h3 className="text-sm font-black uppercase italic text-text-main-light dark:text-white mb-2">Instale no seu iPhone</h3>
            <p className="text-[10px] font-bold text-text-sub-light dark:text-text-sub-dark leading-relaxed mb-6">
              Toque no botão <span className="bg-gray-100 dark:bg-white/10 px-1 rounded inline-flex items-center"><span className="material-symbols-outlined !text-[12px]">ios_share</span> compartilhar</span> abaixo e selecione <br/>
          ...
          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-white/90 dark:bg-surface-dark/90 rotate-45 border-r-2 border-b-2 border-primary/20"></div>
        </div>
      )}
    </>
  );
